...
class JCWAutoTest0021(JcwCase, unittest.TestCase):
    '''
        Automated test case 
    ...
    def custom_setUp(self):
        ...
        deptMainPage.search(self.data.get('ableDept').get('deptName'))
        
    def custom_step(self):
        deptMainPage = DeptMainPage()
        deptMainPage.enterFunction()
        deptMainPage.ableDept({'deptName':self.data.get('ableDept').get('deptName')})
        deptMainPage.search(self.data.get('ableDept').get('deptName'))
        return deptMainPage.checkupTableData(self.data.get('ableDept'))
