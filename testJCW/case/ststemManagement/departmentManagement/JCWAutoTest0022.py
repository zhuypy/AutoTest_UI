...
class JCWAutoTest0022(JcwCase, unittest.TestCase):
    '''
        Automated test case 
    ...
    def custom_setUp(self):
        ...
        deptMainPage.search(self.data.get('disableDept').get('deptName'))
        
    def custom_step(self):
        deptMainPage = DeptMainPage()
        deptMainPage.enterFunction()
        deptMainPage.disableDept({'deptName':self.data.get('disableDept').get('deptName')})
        deptMainPage.search(self.data.get('disableDept').get('deptName'))
        return deptMainPage.checkupTableData(self.data.get('disableDept'))
